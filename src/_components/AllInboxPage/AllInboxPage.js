...
export default function AllInboxPage() {
  ...
  const SeleactAllInBoxCheckBox = (e) => {
    if (e.target.checked) {
      ...
      SetInboxChecked(AllInBoxList.map(item => item._id));
    } else {
      SetSelectAllCheckbox(false);
      SetInboxChecked([]);
    }
        ...
        <Row className='mb-columfull'>
          <Col className='maxcontainerix' id="navclose">
            ...
            <div className='navsmaller px-0 leftinbox'>
              ...
              {
                AllInBoxList.length === 0 ?
                  <div id="scrollableDiv" class="listinbox">
                    <InfiniteScroll
                      dataLength={AllInBoxList.length}
                      next={FetchMoreData}
                      hasMore={false}
                      loader={<h4></h4>}
                    ...
                    <div id="scrollableDiv" class="listinbox">
                      ...
                      >
                        <Stack spacing={1} align="left">
                          {AllInBoxList?.length >= 1 && AllInBoxList?.map((row, index) => (
