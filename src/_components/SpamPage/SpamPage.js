...
export default function SpamPage() {
  ...
  const [SearchSpam, SetSearchSpam] = React.useState("");
  ...
  useEffect(() => {
    ...
    GetClientID();
    GetSpamList();
    if (SpamList?.length > 0) SetSelectedDropdownList(SpamList)
  }, [SearchSpam, ClientID, SpamList?.length,SpamChecked]);
        ...
        >
          <Box sx={Style} className="modal-prein">
            <div className='p-5 text-center'>
              ...
              <Typography id="modal-modal-description" sx={{ mt: 2 }}>
                Are you sure  for move this E-mail into Other Spam ?
              </Typography>
            </div>
            <div className='d-flex btn-50'>
                      ...
                      <div className="userdropall" id="id_userboxlist" ref={WrapperRef}>
                        <div className="bodyuserdop textdeclist">
                          <List dense sx={{ width: '100%', maxWidth: 360, bgcolor: 'background.paper' }}>
                            {SpamList?.map((item) => { // dropdown list
